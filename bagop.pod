...
Add to the backup. Mark I<files> for backing up from now on. Adding a
dir will add all files and furture files under that dir.


=item B<del> I<files>

Delete from the backup. Mark I<files> for not being backed up any
...
=head1 EXAMPLE: add files recursively

Mark multiple files inside directories recursively for backup using I<find> and I<xargs>:

B<find> . -name "*.txt" | B<xargs> B<bagop> add
